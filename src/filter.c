...
float** nlmFilter(float** image, int width, int height, float sigma, int window_size, int patch_size) {
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            ...
            int startRow = std::max(0, i - window_size/2);
            int startColumn = std::max(0, j - window_size/2);
            int endRow = std::min(height - window_size/2, i + window_size/2);
            int endColumn = std::min(width - window_size/2, j + window_size/2);
            ...
            for (int m = startRow; m <= endRow; m++) {
                for (int n = startColumn; n <= endColumn; n++) {
                    ...
                    for (int k = 0; k < patch_size; k++) {
                        ...
                        // printf("distance1 = %f\n", distance);
                        distance /= (patch_size * patch_size);
                        // printf("distance2 = %f\n", distance);
